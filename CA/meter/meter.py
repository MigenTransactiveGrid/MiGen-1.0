...
def run(ID, mRID, LFDI):
    ...
    MirrorUsagePoint = MirrorUsagePoint.toxml(element_name = "MirrorUsagePoint", root_only=True)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ip, port, path, dcap = dns.SepServerFinder(Config.DNS_ADDRESS_M)
            break
        except:
            print 'Error M01, DNS cannot find the requested type'
            logger.info("Error M01, DNS cannot find the requested type")
            time.sleep(5)
    if ip != None:
        while True:
            try:
                ...
                print "Error M02, Cannot discover the SEP server"
                logger.info("Error M02, Cannot discover the SEP server")
                time.sleep(5)
        ...
        while True:
            try:
                ...
                print "Error M03, Cannot retrieve the PIN"
                logger.info("Error M03, Cannot retrieve the PIN")
                time.sleep(5)
    if pIN == PIN:
        while True:
            try:
                run(int(RegistrationLink.split('/edev/')[1].split('/rg')[0]), mRID, LFDI)
                print 'New power metering is posted!'
                logger.info("New power metering is posted!")
                time.sleep((Config.METER)*60)
            except Exception as e:
                print "Error M04, Cannot post the metering values"
                logger.info("Error M04, Cannot post the metering values" + str(e))
                logging.error('Error M04, Cannot post the metering values: ' + str(e))
                time.sleep(5)
    else:
        print "Error M00, device is not registered! Check PIN"
        logger.info("Error M00, device is not registered! Check PIN")
